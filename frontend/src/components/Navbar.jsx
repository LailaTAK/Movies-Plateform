...
import {HelpCircle, Search, Settings, LogOut
...
const Navbar = () => {
  ...
  const handelLogout = async (params) => {
      ...
      <Link to={"/"}>
        ...
        <div className='flex items-center space-x-4 relative'>
          <div className='relative hidden md:inline-flex'>
            <input  type="text" className='bg-[#333333] px-4 py-2 rounded-full *:min-w-72
             pr-10 outline-none' placeholder="Search..."/>
          ...
          {!user ? <Link to={"/signin"}>
          <button className='border border-[#333333] py-2 px-4
          cursor-pointer'>Sign In</button>
          </Link> : <div className='text-white'>
            <img src={avatarUrl} alt="" className='w-10 h-10 rounded-full border-2 border-[#e50914] cursor-pointer'
            onClick={() => setShowMenu(!showMenu)}/>

            {showMenu &&
              <div className="absolute right-0 mt-2 w-64 bg-[#232323] bg-opacity-95
               rounded-lg z-50 shadow-lg py-4 px-3 flex flex-col gap-2 border border-[#333333]" >
              <div className='flex flex-col items-center mb-2'>
                <span className='text-white font-semibold text-base'>{user.username}</span>
                <span className='text-xs text-gray-400'>{user.email}</span>
              </div>

              <button className='flex items-center px-4 py-3 rounded-lg text-white
               bg-[#181818] hover:bg-[#1d1c1c] gap-3 cursor-pointer'>
              ...
              <button onClick={handelLogout} className='flex items-center px-4 py-3 rounded-lg text-white
               bg-[#181818] hover:bg-[#1d1c1c] gap-3 cursor-pointer'>
